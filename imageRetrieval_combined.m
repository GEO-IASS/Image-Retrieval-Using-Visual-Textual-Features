...
% exa   mple usage -- [closestMatches] = imageRetrieval_combined('img_bags_clutch_1.jpg','/Users/tlberg/Desktop/teaching/Fall_12/hw/hw1/images/','/Users/tlberg/Desktop/teaching/Fall_12/hw/hw1/queryimages/');
...
for i = 1:length(db_text)
    for j = 1:length(uniq_words)
        if (allsum(i) ~= 0)
            db_count(i,j) = db_count(i,j) / allsum(i);
        end
    end
end
...
for j = 1:length(query_count)
    query_count(1,j) = query_count(1,j) / query_sum;
end


% compute image based SSD and text based SSD values here
for i = 1:length(db_images)
    ssd_img(i) = sum(sum((query_img-img{i}).^2));
end

for i = 1:length(db_text)
    ssd_txt(i)=0;
    for j = 1:length(uniq_words)
        ssd_txt(i) = ssd_txt(i) + ((query_count(1,j) - db_count(i,j)) * (query_count(1,j) - db_count(i,j)));
    end
    alpha = (ssd_img(i)/(ssd_img(i) + ssd_txt(i))) * 100;
    if((ssd_img(i) == 0) && (ssd_txt(i) ==0) )
        ssd_comb(i) = 0;
    else
        ssd_comb(i) = (((100-alpha) * ssd_img(i)) + (alpha * ssd_txt(i))) / 100;
    end
end
...
fid = fopen('imageRetrieval_combined.html','w');
